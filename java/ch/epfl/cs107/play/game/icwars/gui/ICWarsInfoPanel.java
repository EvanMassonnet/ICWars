...
public class ICWarsInfoPanel implements Graphics {
    ...
    private final ImageGraphics woodSprite, plainSprite, citySprite, mountSprite, riverSprite, roadSprite, lavaSprite, iceSprite;
    ...
    public ICWarsInfoPanel(float cameraScaleFactor) {
        ...
        Shape cellDetailsRect = new Polygon(0,0, 0,height, width,height, width,0);
        Shape unitDetailsRect = new Polygon(0,0, 0,height, width*3,height, width*3,0);

        cellDetailsBackground = new ShapeGraphics(cellDetailsRect, Color.DARK_GRAY, Color.BLACK, 0f, .8f, 3000f);
        unitDetailsBackground = new ShapeGraphics(unitDetailsRect, Color.DARK_GRAY, Color.BLACK, 0f, .8f, 3000f);
        ...
        lavaSprite = new ImageGraphics(ResourcePath.getSprite("icwars/lava"), 1f, 1f, null, anchor, 1f, 3001f);
        iceSprite = new ImageGraphics(ResourcePath.getSprite("icwars/ice"), 1f, 1f, null, anchor, 1f, 3001f);
    ...
    private void drawCellDetails(Canvas canvas, float height, float width) {
        ...
        switch (cellType) {
            ...
            case ROAD:
                ...
                break;
            case LAVA:
                lavaSprite.setRelativeTransform(spriteTransform);
                lavaSprite.draw(canvas);
                break;
            case ICE:
                iceSprite.setRelativeTransform(spriteTransform);
                iceSprite.draw(canvas);
                break;
            default:
    ...
    private void drawUnitDetails(Canvas canvas, float height, float width) {
        final Transform transform = Transform.I.translated(canvas.getPosition().add(width/4 - width/4 , -height/2));
        unitDetailsBackground.setRelativeTransform(transform);
        unitDetailsBackground.draw(canvas);

        final Transform nameTransform = Transform.I.translated(canvas.getPosition().add(width/4+.1f - width/4, -height/4));
